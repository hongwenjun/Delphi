...
uses
  Windows, Graphics, DelphiZXingQRCode, SVGImage, System.UITypes;
...
procedure DrawQR(ACanvas: TCanvas; ARect: TRect; QRCode: TDelphiZXingQRCode; CornerThickness: Integer = 0; QRDrawingMode: TQRDrawingMode = drwRegion; Proportional: Boolean = True);
...
procedure MakeBmp(Bitmap: TBitmap; Scale: Integer; QRCode: TDelphiZXingQRCode; BColor, FColor: TColor; CornerThickness: Integer = 0);
...
procedure MakeMetafile(Metafile: TMetafile; Scale: Integer; QRCode: TDelphiZXingQRCode; BColor, FColor: TColor; QRDrawingMode: TQRDrawingMode = drwRegion; CornerThickness: Integer = 0);

function MakeSvgImagefile(Scale: Integer; QRCode: TDelphiZXingQRCode; BColor, FColor: TColor; QRDrawingMode: TQRDrawingMode = drwRegion; CornerThickness: Integer = 0): TSVGImage;
...
begin
  ...
  H := QRCode.Rows * Scale + CornerThickness * 2;
  quietZone := 5;
  SvgImage := TSVGImage.Create;
  SvgImage.AddHeader;
  SvgImage.AddTag(0, 0, 250, 250, TAlphaColorF.Create(clWhite), TAlphaColorF.Create(clBlack));
  ...
  begin
    ...
    begin
      ...
      begin
        SvgImage.AddRec((Row) * quietZone, (QRCode.Columns - Column) * quietZone, quietZone, quietZone);
      end;
    end;
  end;
  SvgImage.AddEnd;
...
begin
  ...
  begin
    ...
    with Bmp do
    try
      if (ACanvas.Brush.Color = clWhite) and (ACanvas.Brush.Style = bsSolid) and (ACanvas.Pen.Color = clBlack) and (ACanvas.Pen.Mode = pmCopy) then
        Monochrome := True;
  ...
  if CornerThickness > 0 then
    ...
    begin
      Brush.Color := Pen.Color;
      Brush.Style := bsSolid;
      FillRect(Rect(ARect.Right, ARect.Top + H div 2, ARect.Right + CornerThickness, ARect.Bottom));
      FillRect(Rect(ARect.Left + W div 2, ARect.Bottom, ARect.Right + CornerThickness, ARect.Bottom + CornerThickness));
      Brush.Assign(OldBrush);
    end;
end;
